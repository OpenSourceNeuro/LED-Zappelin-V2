...
class Chrolis():
    ...
    def Blanking(self):
        if self.ui.Blanking_toggleButton.isChecked() == True:
            self.ui.Blanking_Toggle_Label.setText("Un-Invert")
            if self.serial_port.is_open:
                self.serial_port.write(str('BO' + '\n').encode('utf-8'))
            else:
                self.ui.Chrolis_Serial_label.setText(
                    'LED Zappelin is not connected: Blanking settings will not be applied')

        if self.ui.Blanking_toggleButton.isChecked() == False:
            self.ui.Blanking_Toggle_Label.setText("Invert")
            if self.serial_port.is_open:
                self.serial_port.write(str('B1' + '\n').encode('utf-8'))
            else:
                self.ui.Chrolis_Serial_label.setText(
                    'LED Zappelin is not connected: Blanking settings will not be applied')

    def BlankingtValue(self):
        self.Blanking_Value = self.ui.Blanking_lineEdit.text()
        if self.serial_port.is_open:
            self.serial_port.write(str('BL ' + str(self.Blanking_Value) + '\n').encode('utf-8'))
            print(self.Blanking_Value)
        else:
            self.ui.Chrolis_Serial_label.setText(
                'LED Zappelin is not connected: Blanking settings will not be applied')
    ...
    def GetChrolisAll(self):
        ...
        self.ChrolisAll_LED_Val = self.ui.ChrolisAll_LED_Slider.value()
        for i in range (self.ui.Chrolis_nLED+1):
            if self.ui.Chrolis_LED_toggleButton[i].isChecked() == True:
                self.ui.Chrolis_LED_Slider[i].setValue(self.ChrolisAll_LED_Val)
                self.ui.Chrolis_LED_Value[i].setText(str(self.ChrolisAll_LED_Val) + ' %')
            if self.serial_port.is_open:
                self.serial_port.write(str('L' + str(i + 1) + (' ') + str(self.ChrolisAll_LED_Val) + '\n').encode('utf-8'))
...
def PlayStimuli(self):
    ...
    def PlayStimulus(self):
        ...
        if self.ui.Chrolis_StartStimulusFlag == True:
            ...
            # self.DaData = ReadSerial(self)
            # self.Data = self.DaData[0]
            # print(self.DaData)
...
class Chrolis_Stimuli():
    ...
    def DrawGraph(self):
        if self.ui.Chrolis_StimulusFlag == True:
            ...
            self.xStim = np.linspace(-int(self.Resolution),int(self.nEntries)*int(self.Resolution),int(self.nEntries))
...
def TestLED(self):
    if self.ui.TestLEDFlag == True:
        ...
        self.ui.Chrolis_Test_pushButton.setStyleSheet("color: rgb(147, 161, 161);\n" "background-color: rgb(7, 54, 66);")
        self.ui.Chrolis_Start_pushButton.setEnabled(True)
        self.ui.Chrolis_Start_pushButton.setStyleSheet("color: rgb(147, 161, 161);\n" "background-color: rgb(7, 54, 66);")
        self.ui.Chrolis_Stop_pushButton.setEnabled(True)
        self.ui.Chrolis_Stop_pushButton.setStyleSheet("color: rgb(147, 161, 161);\n" "background-color: rgb(7, 54, 66);")


    elif self.ui.TestLEDFlag == False:
        self.ui.TestLEDFlag = True
        self.ui.TestLEDPlayFlag = True
        self.ui.Chrolis_Test_pushButton.setStyleSheet("color: rgb(250, 250, 250);\n" "background-color: rgb(220, 50, 47);")
        self.ui.Chrolis_Start_pushButton.setEnabled(False)
        self.ui.Chrolis_Start_pushButton.setStyleSheet("color: rgb(88, 110, 117);\n" "background-color: rgb(0, 30, 38);")
        self.ui.Chrolis_Stop_pushButton.setEnabled(False)
        self.ui.Chrolis_Stop_pushButton.setStyleSheet("color: rgb(88, 110, 117);\n" "background-color: rgb(0, 30, 38);")
